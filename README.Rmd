...
The data frame `namen` contains data on `r nrow(members$namen)` names from `r n_members(members$namen)`:

```{r namen}
members$namen
```

The data frame `bio` contains `r nrow(members$bio)` biographical entries from `r n_members(members$bio)`:

```{r bio}
members$bio
```

The data frame `wp` contains data on `r nrow(members$wp)` parliamentary terms from `r n_members(members$bio)`:

```{r wp}
members$wp
```

The data frame `inst` contains `r nrow(members$inst)` records on institutional membership from `r n_members(members$bio)`:
